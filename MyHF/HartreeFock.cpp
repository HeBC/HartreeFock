...
void HartreeFock::UpdateU_hybrid()
{
    ...
    cblas_dcopy(dim_n * dim_n, FockTerm_n, 1, Heta_n, 1);
    cblas_dscal(dim_n * dim_n, eta, Heta_n, 1);
    for (size_t i = 0; i < dim_n; i++)
    {
    ...
    if (LAPACKE_dsyevd(LAPACK_ROW_MAJOR, 'V', 'L', dim_p, Heta_p, dim_p, energies) != 0)
    { /// 'V' stand for eigenvalues and vectors
        printf("Error when Computes all eigenvalues and eigenvectors in proton Fock term!\n");
        PrintFockMatrix();
        exit(0);
    ...
    // update C
    cblas_dcopy(dim_p * dim_p, U_p, 1, FockTerm_p, 1);
    cblas_dcopy(dim_n * dim_n, U_n, 1, FockTerm_n, 1);
    cblas_dgemm(CblasRowMajor, CblasNoTrans, CblasNoTrans, dim_p, dim_p, dim_p, 1., FockTerm_p, dim_p, Heta_p, dim_p, 0, U_p, dim_p);
    cblas_dgemm(CblasRowMajor, CblasNoTrans, CblasNoTrans, dim_n, dim_n, dim_n, 1., FockTerm_n, dim_n, Heta_n, dim_n, 0, U_n, dim_n);
...
{
    std::cout << cblas_ddot(dim_p, U_p + i, dim_p, U_p + j, dim_p) << std::endl;
}

void HartreeFock::Check_orthogonal_U_n(int i, int j)
{
    std::cout << cblas_ddot(dim_n, U_n + i, dim_n, U_n + j, dim_n) << std::endl;
}
