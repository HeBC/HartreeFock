...
public:
    ...
    double *C_p, *C_n;                /// transformation coefficients, 1st index is ho basis, 2nd = HF basis
    ...
    HartreeFock(Hamiltonian &H, int shift_p, int shift_n); // shift the inital HF orbits
    ~HartreeFock();
    ...
    /// debug code
    void Check_orthogonal_C_p(int i, int j);
    void Check_orthogonal_C_n(int i, int j);
    ...
    : Ham(&H), modelspace(H.GetModelSpace()), tolerance(1e-8)
{
    ...
    this->C_p = (double *)mkl_malloc((dim_p * dim_p) * sizeof(double), 64);
    memset(C_p, 0, (dim_p) * (dim_p) * sizeof(double));
    for (size_t i = 0; i < dim_p; i++)
    {
        C_p[i * (dim_p) + i] = 1.;
    }
    this->C_n = (double *)mkl_malloc((dim_n * dim_n) * sizeof(double), 64);
    memset(C_n, 0, (dim_n) * (dim_n) * sizeof(double));
...
HartreeFock::HartreeFock(Hamiltonian &H, int RandomSeed)
    : Ham(&H), modelspace(H.GetModelSpace()), tolerance(1e-8)
{
    std::cout << "\033[31m  Starting with Random HF states! \033[0m" << std::endl;
    this->N_p = modelspace->GetProtonNum();
    this->N_n = modelspace->GetNeutronNum();
    this->dim_p = modelspace->Get_MScheme_dim(Proton);
    this->dim_n = modelspace->Get_MScheme_dim(Neutron);
    ...
    {
        C_p[i] = (rand() % 1000) / 1000.;
    }
    this->C_n = (double *)mkl_malloc((dim_n * dim_n) * sizeof(double), 64);
    memset(C_n, 0, (dim_n) * (dim_n) * sizeof(double));
    for (size_t i = 0; i < dim_n * dim_n; i++)
    {
        C_n[i] = (rand() % 1000) / 1000.;
    }
    this->FockTerm_p = (double *)mkl_malloc((dim_p) * (dim_p) * sizeof(double), 64);
    memset(FockTerm_p, 0, (dim_p) * (dim_p) * sizeof(double));
    this->FockTerm_n = (double *)mkl_malloc((dim_n) * (dim_n) * sizeof(double), 64);
    ...
    if (LAPACKE_dsyevd(LAPACK_ROW_MAJOR, 'V', 'L', dim_p, C_p, dim_p, energies) != 0)
    { /// 'V' stand for eigenvalues and vectors
        printf("Error when Computes all eigenvalues and eigenvectors in proton Fock term!\n");
        exit(0);
    }
    if (LAPACKE_dsyevd(LAPACK_ROW_MAJOR, 'V', 'L', dim_n, C_n, dim_n, energies) != 0)
    { /// 'V' stand for eigenvalues and vectors
        printf("Error when Computes all eigenvalues and eigenvectors in proton Fock term!\n");
        exit(0);
    }
...
HartreeFock::HartreeFock(Hamiltonian &H, int shift_p, int shift_n)
    : Ham(&H), modelspace(H.GetModelSpace()), tolerance(1e-8)
{
    this->N_p = modelspace->GetProtonNum();
    this->N_n = modelspace->GetNeutronNum();
    ...
    for (size_t i = 0; i < N_p; i++)
    {
        holeorbs_p[i] = SPEpairs_p[i].second + shift_p;
        // std::cout << SPEpairs_p[i].second << std::endl;
    }
    //-----------
    std::vector<std::pair<double, int>> SPEpairs_n(dim_n);
    ...
    for (size_t i = 0; i < N_n; i++)
    {
        holeorbs_n[i] = SPEpairs_n[i].second + shift_n;
    }
    this->energies = (double *)mkl_malloc((dim_p + dim_n) * sizeof(double), 64);
    this->prev_energies = (double *)mkl_malloc((dim_p + dim_n) * sizeof(double), 64);
    memset(energies, 0, (dim_p + dim_n) * sizeof(double));
...
void HartreeFock::UpdateDensityMatrix()
{
    ...
    for (size_t i = 0; i < N_p; i++)
    {
        cblas_dcopy(dim_p, C_p + holeorbs_p[i], dim_p, tmp_p + i, N_p);
    }
    for (size_t i = 0; i < N_n; i++)
    {
        cblas_dcopy(dim_n, C_n + holeorbs_n[i], dim_n, tmp_n + i, N_n);
    }
...
void HartreeFock::UpdateDensityMatrix_DIIS()
{
    ...
    std::vector<double> vec_p(rho_p, rho_p + dim_p * dim_p);
    DIIS_density_mats_p.push_back(vec_p); // in with the new
    ...
    std::vector<double> vec_n(rho_n, rho_n + dim_n * dim_n);
    DIIS_density_mats_n.push_back(vec_n); // in with the new
...
void HartreeFock::Diagonalize()
{
    ...
    ////
    double *e_p, *e_n, *EigenVector_p, *EigenVector_n;

    double alpha = 1;
    double beta = 0;
    e_p = (double *)mkl_malloc((dim_p) * sizeof(double), 64);
    e_n = (double *)mkl_malloc((dim_n) * sizeof(double), 64);
    ...
    if (LAPACKE_dsyevd(LAPACK_ROW_MAJOR, 'V', 'L', dim_p, EigenVector_p, dim_p, e_p) != 0)
    { /// 'V' stand for eigenvalues and vectors
        printf("Error when Computes all eigenvalues and eigenvectors in proton Fock term!\n");
        PrintFockMatrix();
        exit(0);
    ...
    if (LAPACKE_dsyevd(LAPACK_ROW_MAJOR, 'V', 'L', dim_n, EigenVector_n, dim_n, e_n) != 0)
    { /// 'V' stand for eigenvalues and vectors
    ...
    // record energies
    cblas_dcopy(dim_p, e_p, 1, energies, 1);
    cblas_dcopy(dim_n, e_n, 1, energies + dim_p, 1);

    // record C parameters
    cblas_dcopy(dim_p * dim_p, EigenVector_p, 1, C_p, 1);
    cblas_dcopy(dim_n * dim_n, EigenVector_n, 1, C_n, 1);
...
void HartreeFock::Check_orthogonal_C_p(int i, int j)
{
    std::cout << cblas_ddot(dim_p, C_p + i, dim_p, C_p + j, dim_p) << std::endl;
}

void HartreeFock::Check_orthogonal_C_n(int i, int j)
{
    std::cout << cblas_ddot(dim_n, C_n + i, dim_n, C_n + j, dim_n) << std::endl;
}
...
void HartreeFock::PrintParameters_Hole()
{
    ...
    for (size_t j = 0; j < N_p; j++)
    {
        for (size_t i = 0; i < dim_p; i++)
        {
            std::cout << "   " << std::setw(5) << i << "   " << std::setw(5) << j << "       " << std::setw(10) << std::setfill(' ') << std::fixed << std::setprecision(4) << C_p[i * dim_p + j] << std::endl;
        }
    }
    std::cout << std::endl;
    ...
    for (size_t j = 0; j < N_n; j++)
    {
        for (size_t i = 0; i < dim_n; i++)
        {
            std::cout << "   " << std::setw(5) << i << "   " << std::setw(5) << j << "       " << std::setw(10) << std::setfill(' ') << std::fixed << std::setprecision(4) << C_n[i * dim_n + j] << std::endl;
        }
    }
    return;
}
...
void HartreeFock::PrintAllParameters()
{
    ...
    for (size_t i = 0; i < dim_p; i++)
    {
        ...
        {
            std::cout << std::setw(8) << std::setfill(' ') << std::fixed << std::setprecision(4) << C_p[i * dim_p + j];
        }
        std::cout << std::endl;
    }
    std::cout << std::endl;
    ...
    for (size_t i = 0; i < dim_p; i++)
    {
        for (size_t j = 0; j < dim_p; j++)
        {
            std::cout << std::setw(8) << std::setfill(' ') << std::fixed << std::setprecision(4) << C_n[i * dim_p + j];
        }
        std::cout << std::endl;
    }
    std::cout << std::endl;
...
void HartreeFock::PrintAllHFEnergies()
{
    ...
    for (size_t i = 0; i < dim_p; i++)
    {
        std::cout << i << "    " << std::setw(8) << std::setfill(' ') << std::fixed << std::setprecision(15) << prev_energies[i] << std::endl;
    }
    std::cout << std::endl;
    ...
    for (size_t i = 0; i < dim_n; i++)
    {
        std::cout << i << "    " << std::setw(8) << std::setfill(' ') << std::fixed << std::setprecision(15) << prev_energies[i + dim_p] << std::endl;
    }
    std::cout << std::endl;
    return;
}
...
void HartreeFock::SaveHoleParameters(string filename)
{
    ...
    for (size_t i = 0; i < N_p; i++)
        cblas_dcopy(dim_p, C_p + i, dim_p, prt + i * dim_p, 1);
    for (size_t i = 0; i < N_n; i++)
        cblas_dcopy(dim_n, C_n + i, dim_n, prt + i * dim_n + N_p * dim_p, 1);
...
int main(int argc, char *argv[])
{
    ...
    */
    ...
    {
        HartreeFock hf(Hinput, std::stoi(argv[1]), std::stoi(argv[2]));
        hf.Solve();
        hf.SaveHoleParameters("Output/HF_para.dat");
    }
